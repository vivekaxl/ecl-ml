...
EXPORT DATASET(PACF_ACF) LagCorrelations(DATASET(DecObs) post_diff) := FUNCTION
    ...
    ACF_Rec makeACF(LagRec rec, LagRec denom) := TRANSFORM
        ...
        SELF.lags := rec.lags;
    END;
    pre_sumsq := JOIN(r_k_numer, r_k_denom, LEFT.model_id=RIGHT.model_id, makeACF(LEFT, RIGHT), LOOKUP);
    ...
    loop_body(DATASET(Cell) work, UNSIGNED i) := FUNCTION
        work_pairs := JOIN(rk_cells(x<i), work(x=i-1), LEFT.model_id=RIGHT.model_id AND (LEFT.x=i-RIGHT.y OR LEFT.x=RIGHT.y), mult_k_kj(LEFT,RIGHT, i), ALL);
        r_kk := DENORMALIZE(rk_cells(x=i), work_pairs, LEFT.model_id=RIGHT.model_id AND LEFT.x=RIGHT.x, GROUP, make_rkk(LEFT, ROWS(RIGHT)));
        r_kj_r_kk := JOIN(work(x=i-1), r_kk, LEFT.model_id=RIGHT.model_id, reverse_j(LEFT, RIGHT), ALL);
        r_kj := JOIN(work(x=i-1), r_kj_r_kk, LEFT.model_id=RIGHT.model_id AND LEFT.x=RIGHT.x AND LEFT.y=RIGHT.y, reduce_kj(LEFT,RIGHT), LOOKUP);
        RETURN work & r_kk & r_kj;
    END;
    partials := LOOP(init_partial, max_lags, LEFT.lags>=COUNTER, loop_body(ROWS(LEFT), COUNTER));
    Types.PACF_ACF calc_t(ACF_Rec acf, Cell pacf) := TRANSFORM
        ...
        SELF.lags := acf.lags;
    END;
    rslt := JOIN(r_k, partials(x=y), LEFT.model_id=RIGHT.model_id AND LEFT.k=RIGHT.x, calc_t(LEFT, RIGHT), LOOKUP);
    RETURN rslt;
END;
