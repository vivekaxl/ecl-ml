...
EXPORT Identification(DATASET(Types.UniObservation) series, DATASET(Types.Ident_Spec) spec) := MODULE
    ...
    EXPORT DATASET(Types.PACF_ACF) Correlations(UNSIGNED2 lags) := FUNCTION
        ...
        ObsRec addStats(ModelObs obs, RECORDOF(s_stat) st) := TRANSFORM
            ...
            SELF := obs;
        END;
        withStats := JOIN(post_difference, s_stat, LEFT.model_id=RIGHT.model_id, addStats(LEFT, RIGHT), LOOKUP);
