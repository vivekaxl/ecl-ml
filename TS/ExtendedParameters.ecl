...
EXPORT DATASET(Types.Parameter_Extension)ExtendedParameters(DATASET(Model_Parameters) models) := FUNCTION
    ...
    Parm_Extend calcExtend(Model_Parameters prm) := TRANSFORM
        ...
        SELF.theta_phi := JOIN(prm.ar, prm.ma, LEFT.lag=RIGHT.lag, mrgF(LEFT,RIGHT), FULL OUTER);
        SELF.phi := prm.ma;
        SELF := prm;
    END;
