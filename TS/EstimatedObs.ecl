...
UNSIGNED2 forecast_periods=0) := FUNCTION
    ...
    Upd(WorkRec wr, HistRec hr) := MODULE
        ...
        END;
        poly1 := JOIN(wr.theta_phi, act, LEFT.lag = wr.period - RIGHT.period, prodTerm(LEFT,RIGHT, 1), LOOKUP);
        poly2 := JOIN(wr.phi, fct, LEFT.lag = wr.period - RIGHT.period, prodTerm(LEFT,RIGHT,-1), LOOKUP);
