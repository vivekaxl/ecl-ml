...
EXPORT season_adj := MODULE
    ...
    SHARED UniObservation adj_obs(ExtendedObs obs, NumericField f, AdjustPrototype func) := TRANSFORM
        SELF.dependent := IF(f.id=0, obs.dependent, func(f.value, obs.dependent));
        SELF := obs;
    END;
    SHARED DATASET(UniObservation) ApplyAdj(DATASET(UniObservation) ts, DATASET(NumericField) adj, UNSIGNED seasons, UNSIGNED adj_col, AdjustPrototype func) := FUNCTION
        marked := PROJECT(ts, markObs(LEFT, seasons));
        filtered := adj(id<=seasons AND number=adj_col);
        RETURN JOIN(marked, filtered, LEFT.season=RIGHT.id, adj_obs(LEFT, RIGHT, func), LOOKUP);
    END;
    /**
    ...
    */
    EXPORT DATASET(UniObservation) Multiply(DATASET(UniObservation) ts, DATASET(NumericField) adj, UNSIGNED seasons, UNSIGNED adj_col) := FUNCTION
    ...
    * @param adj_col the column number of the seasonal factor
    */
    EXPORT DATASET(UniObservation) Add(DATASET(UniObservation) ts, DATASET(NumericField) adj, UNSIGNeD seasons, UNSIGNED adj_col) := FUNCTION
