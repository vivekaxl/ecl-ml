...
EXPORT DATASET(Part) HadamardProduct(IMatrix_Map x_map, DATASET(Part) X, DATASET(Part) Y) := FUNCTION
    ...
    Part mulPart(Part xrec, Part yrec) := TRANSFORM
        ...
        block_cols:= IF(NOT haveY OR part_cols=yrec.part_cols,part_cols,FAIL(UNSIGNED4, Dimension_IncompatZ, Dimension_Incompat));
        block_rows:= IF(NOT haveY OR part_rows=yrec.part_rows,part_rows,FAIL(UNSIGNED4, Dimension_IncompatZ, Dimension_Incompat));
        ...
        SELF.mat_part := MAP(haveX AND haveY =>  SET(new_elems, v),haveX=> PBblas.MakeR8Set(xrec.part_rows, xrec.part_cols, xrec.first_row, xrec.first_col,
