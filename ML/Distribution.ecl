...
EXPORT Distribution := MODULE
    ...
    EXPORT Uniform(ML.Types.t_FieldReal low,ML.Types.t_FieldReal high,ML.Types.t_Count NRanges = 10000) := MODULE(Default)
        ...
        EXPORT DensityV() := PROJECT(DVec(Nranges,low,RangeWidth),TRANSFORM(Layout,
                                                                                   SELF.P := Density( (LEFT.RangeLow+LEFT.RangeHigh)/2 ),
                                                                                   SELF := LEFT)); // Take density from mid-point
        EXPORT CumulativeV() := PROJECT(DVec(NRanges,low,RangeWidth), TRANSFORM(Layout,SELF.P := Cumulative(LEFT.RangeHigh),SELF := LEFT));
    END;
        ...
        EXPORT Normal(ML.Types.t_FieldReal mean,ML.Types.t_FieldReal sd,ML.Types.t_Count NRanges = 10000) := MODULE(Default)
            ...
            END;
            EXPORT DensityV() := PROJECT(DVec(NRanges,low,RangeWidth),TRANSFORM(Layout,SELF.P := Density((LEFT.RangeLow+LEFT.RangeHigh)/2),SELF := LEFT));
            EXPORT CumulativeV() := PROJECT(DVec(NRanges,low,RangeWidth),TRANSFORM(Layout,SELF.P := Cumulative(LEFT.RangeHigh),SELF := LEFT));
        END;
    ...
    EXPORT DensityV() := PROJECT(DVec(NRanges,low,RangeWidth),
        TRANSFORM(Layout, SELF.P := Density((LEFT.RangeLow+LEFT.RangeHigh)/2), SELF := LEFT));
        EXPORT CumulativeV() := FUNCTION
            d := DensityV();
    ...
    EXPORT Exponential(ML.Types.t_FieldReal lamda,ML.Types.t_Count NRanges = 10000) := MODULE(Default)
        ...
        EXPORT DensityV() := PROJECT(DVec(NRanges,0,RangeWidth), TRANSFORM(Layout,SELF.P := Density((LEFT.RangeHigh+LEFT.RangeLow)/2), SELF := LEFT));
        EXPORT CumulativeV() := PROJECT(DVec(NRanges,0,RangeWidth), TRANSFORM(Layout,SELF.P := Cumulative(LEFT.RangeHigh), SELF := LEFT));
    END;
    ...
    EXPORT Binomial(ML.Types.t_FieldReal p,ML.Types.t_Count NRanges = 100) := MODULE(Default)
        ...
        EXPORT DensityV() := PROJECT(DVec(NRanges,-1,1), TRANSFORM(Layout,SELF.P := Density(LEFT.RangeHigh), SELF := LEFT));
    ...
    EXPORT Poisson(ML.Types.t_FieldReal lamda,ML.Types.t_Count NRanges = 100) := MODULE(Default)
        ...
        EXPORT DensityV() := PROJECT(DVec(NRanges,-1,1), TRANSFORM(Layout,SELF.P := Density(LEFT.RangeNumber-1), SELF := LEFT));
        // Uses the 'default' integration module to construct the cumulative values
        EXPORT Discrete := TRUE;
    END;
