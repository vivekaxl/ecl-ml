...
Types.Element unitDiag(Types.Element elm) := TRANSFORM
  ...
  SELF := elm;
END;
EXPORT LComp(DATASET(Types.Element) l_u) := PROJECT(ML_Mat.LowerTriangle(l_u), unitDiag(LEFT));
EXPORT UComp(DATASET(Types.Element) l_u) := ML_Mat.UpperTriangle(l_u);
...
EXPORT QComp(DATASET(Types.Element) matrix) := ML_Mat.MU.From(QR(matrix), qr_comp.Q);
EXPORT RComp(DATASET(Types.Element) matrix) := ML_Mat.MU.From(QR(matrix), qr_comp.R);;
