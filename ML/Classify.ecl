...
EXPORT Classify := MODULE
    ...
    */
    ...
    EXPORT Logistic_Model := MODULE(DEFAULT), VIRTUAL
        ...
        EXPORT DevianceC(DATASET(ML.Types.NumericField) Indep,DATASET(ML.Types.DiscreteField) Dep, DATASET(ML.Types.NumericField) mod) := MODULE
            ...
            EXPORT AIC := PROJECT(ResidDev, TRANSFORM({ML.Types.t_FieldNumber classifier, REAL8 AIC}, SELF.AIC := LEFT.Deviance + 2 * p; SELF := LEFT));
        END;
        ...
        EXPORT DevianceD(DATASET(ML.Types.DiscreteField) Indep, DATASET(ML.Types.DiscreteField) Dep, DATASET(ML.Types.NumericField) mod) := DevianceC(PROJECT(Indep, ML.Types.NumericField), Dep, mod);
    END;
    /*
    /*
    ...
    */
    EXPORT Logistic(REAL8 Ridge=0.00001, REAL8 Epsilon=0.000000001, UNSIGNED2 MaxIter=200,UNSIGNED4 prows=0, UNSIGNED4 pcols=0,UNSIGNED4 Maxrows=0, UNSIGNED4 Maxcols=0) := MODULE(Logistic_Model)
        Logis(DATASET(ML.Types.NumericField) X, DATASET(ML.Types.NumericField) Y) := MODULE
    ...
    */
    ...
    EXPORT SVM(SVM.Types.SVM_Type svm_type, SVM.Types.Kernel_Type kernel_type,INTEGER4 degree, REAL8 gamma, REAL8 coef0, REAL8 cost, REAL8 eps, REAL8 nu, REAL8 p, BOOLEAN shrinking) := MODULE(DEFAULT)
        SVM.Types.Training_Parameters
        makeParm(UNSIGNED4 dep_field, SVM.Types.SVM_Type svm_type,
        SVM.Types.Kernel_Type kernel_type,
        INTEGER4 degree, REAL8 gamma, REAL8 coef0, REAL8 cost,REAL8 eps, REAL8 nu, REAL8 p, BOOLEAN shrinking) := TRANSFORM
            SELF.id := dep_field;
            ...
            SELF := [];
        END;
        SHARED Training_Param(UNSIGNED4 df) := ROW(makeParm(df, svm_type,kernel_type, degree,gamma, coef0, cost, eps, nu,p, shrinking));
        // Learn from continuous data
