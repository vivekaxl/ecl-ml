    ...
    */
    ...
    EXPORT SplitsInfoGainRatioBased(DATASET( ML.Types.DiscreteField) indep, DATASET( ML.Types.DiscreteField) dep) := FUNCTION
        ...
        PartitionInfoGainRatioBased    (DATASET(NodeInstDiscrete) nodes, ML.Types.t_level p_level) := FUNCTION
            ...
            top_dep_p:= JOIN(top_dep, top_dep_tot, LEFT.node_id = RIGHT.node_id, TRANSFORM(tdp,
                                                                                        SELF.prop:= LEFT.cnt/RIGHT.tot,
                                                                                        SELF.plogp:= P_LOG_P(LEFT.cnt/RIGHT.tot),
                                                                                        SELF:=LEFT),
                                                                                        LOOKUP);
            ...
            pass_thru:= JOIN(top_dep, PureNodes, LEFT.node_id=RIGHT.node_id, TRANSFORM(NodeInstDiscrete,
                                                                                                    SELF.node_id:= LEFT.node_id,
                                                                                                    SELF.level:= p_level,
                                                                                                    SELF.depend:=LEFT.depend,
                                                                                                    SELF.support:=LEFT.cnt,
                                                                                                    SELF.id:=0,
                                                                                                    SELF.number:=0,
                                                                                                    SELF.value:=0),
                                                                                                    LOOKUP);
            ...
            csplit_p:= JOIN(child_tot, top_dep_tot, LEFT.node_id = RIGHT.node_id, TRANSFORM(csp,
                                                                                            SELF.prop:= LEFT.tot/RIGHT.tot,
                                                                                            SELF.plogp:= P_LOG_P(LEFT.tot/RIGHT.tot),
                                                                                            SELF:=LEFT)
                                                                                        );
            ...
            cprop := JOIN(child, child_tot, LEFT.node_id=RIGHT.node_id AND LEFT.number=RIGHT.number AND LEFT.value = RIGHT.value,TRANSFORM(chp,
                                                                                                                        SELF.prop := LEFT.cnt/RIGHT.tot,
                                                                                                                        SELF.plogp:= P_LOG_P(LEFT.cnt/RIGHT.tot),
                                                                                                                        SELF:=LEFT));
            ...
            gain := JOIN(cinfo, top_info, LEFT.node_id=RIGHT.node_id, TRANSFORM(grec,
                                                                                    SELF.node_id:= LEFT.node_id,
                                                                                    SELF.number:=LEFT.number,
                                                                                    SELF.gain:= RIGHT.info - LEFT.info),
                                                                                    LOOKUP);
            grrec := RECORD
                ML.Types.t_node node_id;
                ML.Types.t_Discrete number;
            ...
            gainRatio := JOIN(gain, csplit, LEFT.node_id=RIGHT.node_id AND LEFT.number=RIGHT.number, TRANSFORM(grrec,
                                                                                                            SELF.node_id:= LEFT.node_id,
                                                                                                            SELF.number:=LEFT.number,
                                                                                                            SELF.gain_ratio:= LEFT.gain/RIGHT.split_info));
            ...
            new_split := PROJECT(new_spl0, TRANSFORM(NodeInstDiscrete,
                                                SELF.value:= node_base + COUNTER;
                                                SELF.depend:= LEFT.value;
                                                SELF.level:= p_level;
                                                SELF.support:= LEFT.tot;
                                                SELF := LEFT;
                                                SELF := [];));
            node_inst := JOIN(toSplit, new_split, LEFT.node_id=RIGHT.node_id AND LEFT.number=RIGHT.number AND LEFT.value=RIGHT.depend, TRANSFORM(NodeInstDiscrete,
                                                                                                                                        SELF.node_id:=RIGHT.value,
                                                                                                                                        SELF.level:= RIGHT.level +1,
                                                                                                                                        SELF.value:= LEFT.depend,
                                                                                                                                        SELF:= LEFT ));
    ...
    EXPORT BinaryPartitionC    (DATASET(cNode) nodes, ML.Types.t_level p_level, ML.Types.t_Count minNumObj=2) := FUNCTION
        ...
        MDLcorrection:= JOIN(bag_count, node_dep_tot, LEFT.node_id=RIGHT.node_id, TRANSFORM({bag_Count, REAL IGpenalty},
                                                                                            SELF.IGpenalty:= (LOG(LEFT.bagCnt)/LOG(2))/RIGHT.tot,
                                                                                            SELF:=LEFT), LOOKUP);
        ...
        END;
        // Information Entropy of new branches per split
        bag_gprop:= JOIN(bag_grouped, cuts_ok, LEFT.node_id=RIGHT.node_id AND LEFT.number=RIGHT.number AND LEFT.value = RIGHT.value,TRANSFORM(bag_dep,
                                                                                                                    SELF.prop_low := LEFT.tot_low/RIGHT.tot_low,
                                                                                                                    SELF.plogp_low:= P_LOG_P(LEFT.tot_low/RIGHT.tot_low),
                                                                                                                    SELF.prop_high := LEFT.tot_high/RIGHT.tot_high,
                                                                                                                    SELF.plogp_high:= P_LOG_P(LEFT.tot_high/RIGHT.tot_high),
                                                                                                                    SELF:=LEFT), HASH);
        ...
        bag_gr := RECORD
            ...
            REAL4 g_ratio:= 0;
        END;
        bags_gain:= JOIN(bags_Entropy, node_entropy, LEFT.node_id=RIGHT.node_id, TRANSFORM(bag_gr, SELF.gain:= RIGHT.info - LEFT.info , SELF:= LEFT), LOOKUP);
        ...
        bagSplit_gRatio := JOIN(cuts_ok, bagsplit_MDL(gain > 0), LEFT.node_id=RIGHT.node_id AND LEFT.number=RIGHT.number AND LEFT.value=RIGHT.value, TRANSFORM(bag_gr,
                                                                                                                                            SELF.info:= LEFT.split_info,
                                                                                                                                            SELF.g_ratio:= RIGHT.gain/LEFT.split_info,
                                                                                                                                            SELF:= RIGHT), LOCAL);
