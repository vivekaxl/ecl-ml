...
DATASET(Types.SVM_Instance) ds) := FUNCTION
    ...
    f_stats := TABLE(features, {STRING1 lbl:='1', nominal, REAL8 mx_v := MAX(GROUP, v),REAL8 mn_v:= MIN(GROUP, v)}, nominal, FEW, UNSORTED);
    l_stats := TABLE(ds, {STRING1 lbl:='1', REAL8 mx_v := MAX(GROUP,y), REAL8 mn_v:=MIN(GROUP, y)}, FEW, UNSORTED);
