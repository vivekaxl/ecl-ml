...
EXPORT Converted := MODULE
    ...
    EXPORT DATASET(SVM_Instance) LIBSVMDATA2Instance(STRING fname):=FUNCTION
        ...
        F_Plus checkAscending(F_Plus prev, F_Plus curr) := TRANSFORM
            SELF.good := (NOT prev.flag AND curr.nominal >= 0) OR (curr.nominal >= 0 AND curr.nominal > prev.nominal);
            SELF := curr;
        END;
        STRING delims := ' \t\r\n';
    ...
    EXPORT DATASET(ECL_LibSVM_Problem) Instance2Problem(DATASET(SVM_Instance) ds) := FUNCTION
        ...
        ECL_LibSVM_Problem cvtInstance(SVM_Instance instance) := TRANSFORM
            ...
            SELF.x := PROJECT(SORT(instance.x, nominal), cvtFeature(LEFT)) & DATASET(1, lastNode());
        END;
