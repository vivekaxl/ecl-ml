...
EXPORT Converted := MODULE
    ...
    DATASET(ML_Types.NumericField) Dep=dummy) := FUNCTION
        ...
        SVM.Types.SVM_Feature cvtF(ML_Types.NumericField nf) := TRANSFORM
            ...
            SELF.v := nf.value;
        END;
        SVM.Types.SVM_Instance rollNF(ML_Types.NumericField f, DATASET(ML_Types.NumericField) ds) := TRANSFORM
            SELF.rid := f.id;
            SELF.y := 0;
    ...
    Work1 cvt_2_r8(SVM.Types.SVM_SV s) := TRANSFORM
        SELF.d := DATASET([{2*(COUNT(s.features)+1)},{s.v_ord}], SVM.Types.R8Entry) + NORMALIZE(s.features, 2, norm_feature(LEFT, COUNTER));
    END;
    ...
    EXPORT FromModel(UNSIGNED id_base, DATASET(SVM.Types.Model) mdl) := FUNCTION
        ...
        ML_Types.NumericField normR8(SVM.Types.Model m, SVM.Types.R8Entry d,UNSIGNED c, UNSIGNED4 base) := TRANSFORM
            SELF.id := id_base + m.id;
            SELF.number := base + c;
            SELF.value := d.v;
        END;
        coef_part := NORMALIZE(mdl, LEFT.sv_coef, normR8(LEFT, RIGHT, COUNTER, lf_id + get_SV_Count(LEFT.sv)));
        rho_part  := NORMALIZE(mdl, LEFT.rho, normR8(LEFT, RIGHT, COUNTER, lf_id + get_SV_Count(LEFT.sv) + COUNT(LEFT.sv_coef)));
        probA_part:= NORMALIZE(mdl, LEFT.probA, normR8(LEFT, RIGHT, COUNTER,lf_id + get_SV_Count(LEFT.sv) + COUNT(LEFT.sv_coef) + COUNT(LEFT.rho) ));
        probB_part:= NORMALIZE(mdl, LEFT.probB, normR8(LEFT, RIGHT, COUNTER, lf_id + get_SV_Count(LEFT.sv) + COUNT(LEFT.sv_coef) + COUNT(LEFT.rho) + COUNT(LEFT.probA)  ));
        ML_Types.NumericField normI4(SVM.Types.Model m, SVM.Types.I4Entry d, UNSIGNED c, UNSIGNED4 base) := TRANSFORM
            SELF.id := id_base + m.id;
            SELF.number := base + c;
        ...
        RETURN fixed_part + sv_part + coef_part + rho_part + probA_part + probB_part + lab_part + nsv_part;
    END;
    //
    ...
    Exp_NF mark_xnf(Exp_NF prev, Exp_NF curr) := TRANSFORM
        SELF.vector := IF(curr.number>prev.vector, curr.number+(UNSIGNED)curr.value-1,prev.vector);
