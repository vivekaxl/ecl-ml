...
EXPORT Converted := MODULE
    ...
    END;
    EXPORT FromPart2Cell(DATASET(Layout_Part) part_recs) := NORMALIZE(part_recs, COUNT(LEFT.mat_part), cvtPart2Cell(LEFT, COUNTER));
    ...
    ML_Types.NumericField cvt2NF(Layout_Cell cell) := TRANSFORM
        ...
        SELF.value            := cell.v;
    END;
  EXPORT FromPart2DS(DATASET(Layout_Part) part_recs) := PROJECT(FromPart2Cell(part_recs), cvt2NF(LEFT));
    ...
    Mat_Types.Element cvt(Layout_Cell cell) := TRANSFORM
        ...
        SELF := cell;
    END;
    EXPORT FromPart2Elm(DATASET(Layout_Part) part_recs) := PROJECT(FromPart2Cell(part_recs), cvt(LEFT));
END;